...
DROP TABLE IF EXISTS public.saved_observations CASCADE;
CREATE TABLE public.saved_observations (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users ON DELETE CASCADE,
  experiment_id text NOT NULL,
...
DROP POLICY IF EXISTS "Users can view own stars" ON public.starred_experiments;
CREATE POLICY "Users can view own stars" ON public.starred_experiments FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can bookmark experiments" ON public.starred_experiments;
CREATE POLICY "Users can bookmark experiments" ON public.starred_experiments FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can remove bookmarks" ON public.starred_experiments;
CREATE POLICY "Users can remove bookmarks" ON public.starred_experiments FOR DELETE USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "Users can record visit" ON public.recently_viewed;
CREATE POLICY "Users can record visit" ON public.recently_viewed FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update visit time" ON public.recently_viewed;
CREATE POLICY "Users can update visit time" ON public.recently_viewed FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can clear history" ON public.recently_viewed;
